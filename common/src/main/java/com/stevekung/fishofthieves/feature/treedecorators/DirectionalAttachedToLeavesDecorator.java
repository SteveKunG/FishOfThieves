...
{
    public static final MapCodec<DirectionalAttachedToLeavesDecorator> CODEC = RecordCodecBuilder.mapCodec(instance -> instance.group(
    ...
    public void place(TreeDecorator.Context context)
    {
        ...
        for (var leafPos : Util.shuffledCopy(context.leaves(), randomSource))
        {
            ...
            if (!blacklistSet.contains(placementPos) && randomSource.nextFloat() < this.probability && this.hasRequiredEmptyBlocks(context, leafPos, direction))
            {
                ...
                context.setBlock(placementPos, this.blockProvider.getState(context.level(), randomSource, placementPos, this.opposite ? direction.getOpposite() : direction));
            }
        }
    }
