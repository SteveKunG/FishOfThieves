...
public interface AbstractFishVariant extends PriorityProvider<SpawnContext, SpawnCondition>
{
    ...
    record RegisterContext<T>(String entityName, Function6<String, ClientAsset, Optional<ClientAsset>, Optional<Boolean>, SpawnSettings, Integer, T> factory)
    {
        ...
        public void register(BootstrapContext<T> context, ResourceKey<T> key, String name, int order, boolean glow, Optional<Boolean> treasured, List<Selector<SpawnContext, SpawnCondition>> conditions, Optional<List<Selector<SpawnContext, SpawnCondition>>> fishingOverride)
        {
            ...
            context.register(key, this.factory.apply(name, new ClientAsset(texture), glow ? Optional.of(new ClientAsset(glowTexture)) : Optional.empty(), treasured, new SpawnSettings(new SpawnPrioritySelectors(conditions), fishingOverride.map(SpawnPrioritySelectors::new)), order));
        }
    }
