...
public record FOTLocationPredicate(TagKey<Biome> biome, TagKey<ConfiguredStructureFeature<?, ?>> structure, Continentalness continentalness, Boolean hasRaids)
{
    ...
    public boolean matches(ServerLevel level, double x, double y, double z)
    {
        ...
        var structureFeatureHolderSet = level.registryAccess().registryOrThrow(Registry.CONFIGURED_STRUCTURE_FEATURE_REGISTRY).getTag(this.structure);
    ...
    private boolean structureMatched(ServerLevel level, BlockPos blockPos, HolderSet.Named<ConfiguredStructureFeature<?, ?>> holders)
    {
        for (var holder : holders)
        {
            return level.structureFeatureManager().getStructureWithPieceAt(blockPos, holder.value()).isValid();
        }
        return false;
    }
    ...
    public static FOTLocationPredicate fromJson(@Nullable JsonElement json)
    {
        ...
        TagKey<ConfiguredStructureFeature<?, ?>> structure = null;
        ...
        if (jsonObject.has("structure"))
        {
            ...
            structure = TagKey.create(Registry.CONFIGURED_STRUCTURE_FEATURE_REGISTRY, new ResourceLocation(string));
        }
    ...
    public static class Builder
    {
        ...
        public Builder setStructure(@Nullable TagKey<ConfiguredStructureFeature<?, ?>> structure)
        {
