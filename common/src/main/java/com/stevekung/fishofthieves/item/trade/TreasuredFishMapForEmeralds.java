...
public record TreasuredFishMapForEmeralds(int farEmeraldCost, int nearEmeraldCost, String displayName, int maxUses, int villagerXp, int tier) implements VillagerTrades.ItemListing
{
    ...
    public MerchantOffer getOffer(ServerLevel serverLevel, Entity entity, RandomSource randomSource)
    {
        ...
        var farthest = ShoalSpawner.findFarthest(holder -> holder.is(FOTPoiTypes.NATURAL_SHOAL), entity.blockPosition(), 50, 100, serverLevel.getPoiManager());
        ...
        {
            var attemptPos = ShoalSpawner.attemptSpawnShoal(serverLevel, entity.blockPosition(), 10);
            ...
            {
                var nearest = serverLevel.getPoiManager().findClosest(holder -> holder.is(FOTPoiTypes.NATURAL_SHOAL), entity.blockPosition(), 100, PoiManager.Occupancy.ANY);
