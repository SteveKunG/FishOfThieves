...
public record TreasuredFishMapForEmeralds(int farEmeraldCost, int nearEmeraldCost, String displayName, int maxUses, int villagerXp, int tier) implements VillagerTrades.ItemListing
{
    ...
    {
        var pair = getTreasuredFishMap(serverLevel, entity, this.displayName, this.farEmeraldCost, this.nearEmeraldCost, this.tier);
        return pair == null ? null : new RestockableMerchantOffer(this.tier, new ItemCost(Items.EMERALD, pair.getSecond()), Optional.of(new ItemCost(Items.COMPASS)), pair.getFirst(), this.maxUses, this.villagerXp, 0.2F);
    }
