...
public class PlentifinVariants
{
    ...
    public static void bootstrap(BootstrapContext<PlentifinVariant> context)
    {
        ...
        registerContext.register(context, AMBER, "amber", 1, AllOfCondition.allOf(InvertedSpawnCondition.invert(RainingCondition.raining().build()),
                TimeOfDayCondition.timeOfDay(MinMaxBounds.Doubles.between(0.75d, 0.9d)),
                SeeSkyCondition.seeSky()));
        registerContext.register(context, CLOUDY, "cloudy", 2, AllOfCondition.allOf(RainingCondition.raining().build(), SeeSkyCondition.seeSky()));
        registerContext.register(context, BONEDUST, "bonedust", 3,
                registerContext.select(AllOfCondition.allOf(new StructureCheck(structureLookup.getOrThrow(FOTTags.Structures.BONEDUST_PLENTIFINS_SPAWN_IN)), RandomChanceCondition.chance(10)), 1),
                registerContext.select(ProbabilityCondition.defaultRareProbablity(), 0));
        registerContext.register(context, WATERY, "watery", 4, true, AllOfCondition.allOf(NightCondition.night(), SeeSkyCondition.seeSky()));
    }
