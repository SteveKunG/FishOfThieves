...
public class DevilfishVariants
{
    ...
    public static void bootstrap(BootstrapContext<DevilfishVariant> context)
    {
        ...
        registerContext.register(context, FIRELIGHT, "firelight", 4, true, NightCondition.night()
                .and(MatchBlocksInRangeCondition.blocksInRange(Optional.of(context.lookup(Registries.BLOCK).getOrThrow(FOTTags.Blocks.FIRELIGHT_DEVILFISH_WARM_BLOCKS)), Optional.of(context.lookup(Registries.FLUID).getOrThrow(FluidTags.LAVA)), 4)));
    }
    ...
    public static void bootstrapSimple(BootstrapContext<DevilfishVariant> context)
    {
        ...
        registerContext.register(context, FORSAKEN, "forsaken", 3, registerContext.select(ProbabilityCondition.defaultRareProbablity(), 0));
        registerContext.register(context, FIRELIGHT, "firelight", 4, true, NightCondition.night().and(SeeSkyCondition.seeSky()));
    }
