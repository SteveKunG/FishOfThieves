...
public class WildsplashVariants
{
    ...
    public static void bootstrap(BootstrapContext<WildsplashVariant> context)
    {
        ...
        registerContext.register(context, RUSSET, "russet");
        registerContext.register(context, SANDY, "sandy", MatchBiomeCondition.biomes(biomeLookup.getOrThrow(BiomeTags.IS_BEACH)).and(ContinentalnessCondition.builder().continentalness(Continentalness.COAST)).build());
        registerContext.register(context, OCEAN, "ocean", MatchBiomeCondition.biomes(biomeLookup.getOrThrow(BiomeTags.IS_OCEAN)).build());
        registerContext.register(context, MUDDY, "muddy", AllOfCondition.allOf(ProbabilityCondition.defaultRareProbablity(), MatchBiomeCondition.biomes(biomeLookup.getOrThrow(BiomeTags.HAS_CLOSER_WATER_FOG))).build());
        registerContext.register(context, CORAL, "coral", true, AllOfCondition.allOf(NightCondition.night(), SeeSkyCondition.seeSky(), MatchBiomeCondition.biomes(HolderSet.direct(biomeLookup.getOrThrow(Biomes.WARM_OCEAN))), MatchMinimumBlocksInRangeCondition.minimumBlocksInRange(Optional.of(context.lookup(Registries.BLOCK).getOrThrow(FOTTags.Blocks.CORAL_WILDSPLASH_SPAWNABLE_ON)), Optional.empty(), 4, 24)).build());
    }
    ...
    public static void bootstrapSimple(BootstrapContext<WildsplashVariant> context)
    {
        var registerContext = AbstractFishVariant.RegisterContext.create("wildsplash", WildsplashVariant::new);
        registerContext.register(context, RUSSET, "russet");
        registerContext.register(context, SANDY, "sandy");
        registerContext.register(context, OCEAN, "ocean");
        registerContext.register(context, MUDDY, "muddy", ProbabilityCondition.defaultRareProbablity().build());
        registerContext.register(context, CORAL, "coral", true, AllOfCondition.allOf(NightCondition.night(), SeeSkyCondition.seeSky()).build());
    }
