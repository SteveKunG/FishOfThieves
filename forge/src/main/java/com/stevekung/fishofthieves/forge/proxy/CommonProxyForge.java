...
public class CommonProxyForge
{
    ...
    public void registerLootTables(LootTableLoadEvent event)
    {
        ...
        else if (id.equals(BuiltInLootTables.FISHING_FISH))
        {
            ...
            inject(table, "pool1", pool.entries);
            inject(table, "main", pool.entries);
//            table.addPool(LootPool.lootPool().add(LootItem.lootTableItem(FOTItems.WRECKER)
//                    .setWeight(100)
//                    .when(FOTLootItemConditions.IN_SHIPWRECKS)).build());
        }
    }
